...
def getSystem():
  systemSize = random.randrange(2, 10)
...
def generateEntries(x):
  for i in range(x):
    ...
    with open('C:\\Users\\Adrian\\Desktop\\GenerateLicenta\\licenta_dataset.csv', 'a+', newline='', encoding='UTF8') as f:
        writer = csv.writer(f)
        #writer.writerow(['system','solution','output'])
        writer.writerow([str(syst.tobytes()), str(sol.tobytes()), str(out.tobytes())])
        print("Solved Values [X,Y,Z,W,A,R..ETC]: {} ".format(out))
        f.close()
