...
datasets = ['Neural Network - Systems with a Single Rank/datasets/SystemSize2(18-Decimal)DatasetTest.csv',
            'Neural Network - Systems with a Single Rank/datasets/FloatValueInterval[0,1](18-Decimals)Dataset.csv',
...
def normalizeDataset(X, Y):
    """
    Normalizarea setului de date, pentru sisteme mari, ranguri diferite.
    @param X: coeficientii matricei
    @param Y: necunoscutele
    @return: sistemele normalizate in intervalul [0,1].
    """
    newX = []
    newY = []
    ...
    print(X[0])
    for list in X:
        listNewX = preprocessing.normalize([list])
        newX.append(listNewX)

    for list in Y:
        listNewY = preprocessing.normalize([list])
        newY.append(listNewY)

    return np.asarray(newX), np.asarray(newY)

# Apelarea metodelor, pentru a vedea diferite informatii despre sistemele de date.
