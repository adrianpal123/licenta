...
DATASET = pd.read_csv('datasets/FloatValueInterval[0,1](18-Decimals)-Size[2-20]Dataset.csv')
...
def neuralNetworkModel(dataset):
    ...
    model = Sequential()
    model.add(Flatten(input_shape=(19, 20)))
    ...
    history = model.fit(X_train, Y_train, epochs=100, batch_size=128, verbose='auto', validation_split=0.2,
                        shuffle=True)
